...
a01d.nbloc  =toVector(prefs[prefs$name=='a01d.nbloc',"values"])
  ...
  ncii.set  =toVector(prefs[prefs$name=='ncii.set',"values"])
  ncii.nreps  =toVector(prefs[prefs$name=='ncii.nreps',"values"])
  ncii.adf.name  =toVector(prefs[prefs$name=='ncii.adf.name',"values"])
  ncii.adf.level  =toVector(prefs[prefs$name=='ncii.adf.level',"values"])

  lxt.nreps  =toVector(prefs[prefs$name=='lxt.nreps',"values"])
  lxt.adf.name  =toVector(prefs[prefs$name=='lxt.adf.name',"values"])
  lxt.adf.level  =toVector(prefs[prefs$name=='lxt.adf.level',"values"])
					  ...
					  ,list(label="NCII",
					    ...
					    children=list(
					      list(name="ncii.set",
					        ...
					        items=ncii.set,
					        selected = 
					          which(ncii.set==prefs[prefs$name=='ncii.set',"past"])
					      ),
					      list(name="ncii.nreps",
					        #label=prefs[prefs$name=="nreps","label_en"],
					        label = translate("_LBL_N_REPS_"),
					        type="gcombobox",
					        items=ncii.nreps,
					        selected = 
					          which(ncii.nreps==prefs[prefs$name=='ncii.nreps',"past"])
					      ),
					      list(name="ncii.adf.name",
					        type="gedit",
					        #label=prefs[prefs$name=='f2rcbd.adf.name',"label_en"],
					        label = translate("_LBL_F2_"),
					        text=prefs[prefs$name=='ncii.adf.name',"past"]
					      ),
					      list(name="ncii.adf.level",
					        type="gtext",
					        #label=prefs[prefs$name=='f2rcbd.adf.level',"label_en"],
					        label = translate("_LBL_F2_LEVEL_"),
					        label.pos = "top",
					  ...
					  ,list(label="LXT",
					    ...
					    children=list(
					      list(name="lxt.nreps",
					        ...
					        items=lxt.nreps,
					        selected = 
					          which(lxt.nreps==prefs[prefs$name=='lxt.nreps',"past"])
					      ),
					      list(name="lxt.adf.name",
					        type="gedit",
					        #label=prefs[prefs$name=='f2rcbd.adf.name',"label_en"],
					        label = translate("_LBL_F2_"),
					        text=prefs[prefs$name=='lxt.adf.name',"past"]
					      ),
					      list(name="lxt.adf.level",
					        type="gtext",
					        #label=prefs[prefs$name=='f2rcbd.adf.level',"label_en"],
					        label = translate("_LBL_F2_LEVEL_"),
					        label.pos = "top",
...
#	}
  ...
  if(str_detect(out$sdesign,"(NCII)")){
    adfn = out$ncii.adf.name
    nreps= as.integer(out$ncii.nreps)
    bsize= as.integer(out$ncii.set)
    adfl = str_split(out$ncii.adf.level,"\n")[[1]]
  }

  if(str_detect(out$sdesign,"(LXT)")){
    adfn = out$lxt.adf.name
    nreps= as.integer(out$lxt.nreps)
    adfl = str_split(out$lxt.adf.level,"\n")[[1]]
  }
...
#	abbr = dict$Abbreviations%in%variab
    ...
    # Aqui realiza la randomizacion segun la eleccion de los componentes en la interfaz
      ...
      #{
        #Sheets=c("F1_Selec_Crit","F2 Clones_Select_Flowering","F3_Clones_Selec_Harvest","F4_ Harvest_Mother", "F5_harvest_Baby", "F6_organoleptic_rural", "F7_Dormancy_weight_loss_tuber", "F8_Selec_clone_Storage ")
		    ...
		    Sheets=c("F1_Selec_Crit","F2 Clones_Select_Flowering","F3_Clones_Selec_Harvest","F4_ Harvest_Mother", "F5_harvest_Baby", "F6_Mother_Organoleptic","F7_Baby_Organoleptic","F8_Dormancy_weight_loss_tuber","F9_Selec_clone_Storage ")
        ...
        #add.vals.columns(archivo,Sheet1=Sheets[2],mat=fbd,IniR=2)
        #add.vals.columns(archivo,Sheet1=Sheets[3],mat=fbd,IniR=2)
        #add.vals.columns(archivo,Sheet1=Sheets[4],mat=fbd,IniR=2)
        #add.vals.columns(archivo,Sheet1=Sheets[5],mat=fbd,IniR=2)
        
        #A=c("Panel No :", "Type of trial:",  "Name of Evaluator :",  "Sex :"," ","Factor",  	"Appearance",	"Excellent",	"Fair",	"Poor",	"Taste", "Excellent",	"Fair",	"Poor",	"Texture",	"Mealy (floury)",	"Intermediate",	"Soggy (=watery)")
        #B=c(rep(" ",5),"Grade"," ",5,3,1," ",5,3,1," ",9,6,3)
        ...
        #add.vals.DF(archivo,Sheet1=Sheets[6],AB1,IniC=1,IniR=1,xVal=1,COLOR1="orange",col.n=FALSE, row.n=FALSE)
        #add.vals.DF(archivo,Sheet1=Sheets[6],AB2,IniC=1,IniR=6,xVal=1,COLOR1="red",col.n=FALSE, row.n=FALSE)
        add.vals.DF(archivo,Sheet1=Sheets[6],Cmat,IniC=3,IniR=7,xVal=1,COLOR1="red", row.n=FALSE)
        
        #add.vals.columns(archivo,Sheet1=Sheets[6],mat=Cmat)
      ...
      #}else{
        ...
        #saveWorkbook(wb,to)
		    ...
		    if(out$logPrefix!="PTPV") ## Condicional para omitir en caso de M&B (reemplazar metodo)
		    {
		      wb <- loadWorkbook(to)
        ...
        #saveWorkbook(wb,to)
  		  ...
  		  if(out$logPrefix!="PTPV") ## Condicional para omitir en caso de M&B (reemplazar metodo)
  		  {
  			...
  			#add.fieldbook(data,wb,prefs, vss, out$logPrefix,"Fieldbook2" ) ## ya no se crea fieldbook2
  			#saveWorkbook(wb,to)
      #}
...
createDesignExpDlg <- function(win){
	...
	#print("1")
	log.list = doLayoutExp(prefs) ## crea las 1ras solapas
	#print("2")
	...
	wg = ggroup(cont=g)
	wdg = gformlayout(log.list, container=wg) ## obtiene lo que se selecciona en la ventana de las solapas
	#print("3")
	#print(svalue(wdg))
	bg <- ggroup(cont = g)
	addSpring(bg)
...
#				#print(nchar(res))
				if(nchar(msg)==0){
					...
					if(gconfirm(msg,translate("_MSG_CONFIRM_"),"info")){
						...
						buildDesign(out) ## aqui se estan agregando las nuevas hojas / y la randomizacion
